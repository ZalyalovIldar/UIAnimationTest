...
class Branch: NSObject {
    ...
    init(normalLine: Line!, angleBetweenLines angelA: CGFloat! = 60.0, percentageOfCompression percent: CGFloat! = 0) {
        ...
        rightLine = Line(startPoint: normalLine.endPoint, endPoint: CGPoint(x: x1, y: y1), strokeWidth: strokeWidth)
        leftLine = Line(startPoint: normalLine.endPoint, endPoint: CGPoint(x: x2, y: y2), strokeWidth: strokeWidth)
    }
    
    func drawInContext(context: CGContext!, withColor color: UIColor! = UIColor.black()) {
        self.leftLine.drawInContext(context: context, withColor: color)
        self.rightLine.drawInContext(context: context, withColor: color)
    }
